(function(){
  ...
  function createTodoItem(todoItemData, {onDone, onDelete}) {
    ...
    // dz
    if (todoItemData.done === true) item.classList.add('list-group-item-success');
  ...
  // создает список дел
  async function createTodoApp(container, title ='Список дел', keyName, arrDop=[]){
    // вызываем функции
    ...
    todoItemList.forEach(todoItemData => {
      const todoItemElement = createTodoItem(todoItemData, handlers);
